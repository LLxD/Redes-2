...
public class RTPpacket {
  ...
  public RTPpacket(int PType, int Framenb, int Time, byte[] data, int data_length) {
    ...
    // O campo de versão foi deslocado para ser colocado no cabeçalho
    header[0] = (byte) (Version << 6);

    // O campo ptype é colocado no cabeçalho
    header[1] = (byte) (PType & 0b01111011);

    // O número de sequência é colocado no cabeçalho
    header[2] = (byte) (SequenceNumber >> 8);
    header[3] = (byte) (SequenceNumber & 0b11111111);

    // O tempo de vídeo é colocado no cabeçalho
    header[4] = (byte) (TimeStamp >> 24);
    header[5] = (byte) (TimeStamp >> 16);
    header[6] = (byte) (TimeStamp >> 8);
    header[7] = (byte) (TimeStamp & 0b11111111);

    // O SSRC é colocado no cabeçalho
